  ...
  async (req, res, next) => {
    ...
    if (!roster) {
      return res.status(400).json({
        errorMessage: `해당하는 플레이어가 로스터에 존재하지 않습니다.`,
      });
    }

    const team = await prisma.squads.findFirst({
      where: {
        playerId: playerId,
        accountId: accountId,
      },
    ...
    if (team) {
      return res.status(400).json({
        errorMessage: `동일한 플레이어를 팀에 여러명 편성할 수는 없습니다.`,
      });
    }
    ...
    return res
      .status(200)
      .json({ message: `${player.name}이 팀에 추가되었습니다.` });
  },
);
